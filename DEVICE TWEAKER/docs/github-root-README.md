...
После сборки пакет появится в `DEVICE TWEAKER\bin\ReleasePackages\<version>\`.

Подробная инструкция: [`DEVICE TWEAKER/docs/BUILD.md`](./DEVICE%20TWEAKER/docs/BUILD.md). История изменений: [`DEVICE TWEAKER/CHANGELOG.md`](./DEVICE%20TWEAKER/CHANGELOG.md).
...
Исходный код KDU находится в `DEVICE TWEAKER/IMOD/KDU` и распространяется по лицензии MIT.
