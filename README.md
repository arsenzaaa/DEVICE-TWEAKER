...
<p align="center">
  <img src="./DEVICE%20TWEAKER/assets/screenshots/showcase_amd_imod_table_ru.png" alt="Таблица аппаратной модерации таймеров USB xHCI IMOD" width="100%">
</p>

#### Практические примеры конфигурации USB IMOD

| Сценарий применения | Контроллер / Топология | Значение IMOD | Задержка | Приоритет IRQ | Назначение Affinity |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Игровая мышь (1000 - 8000 Гц)** | Процессорный CHIP 0 (Direct CPU) | `0x0` | 0 нс (Zero latency) | Высокий (High) | Выделенное физическое ядро (P-Core) |
| **Игровая клавиатура (1000 - 8000 Гц)** | Чипсетный CHIP 1 (PCH Hub) | `0x0` | 0 нс (Zero latency) | Высокий (High) | Смежное изолированное ядро |
| **USB ЦАП / Внешняя звуковая карта** | Чипсетный CHIP 1 (PCH Hub) | `0xFA0` | 1000 мкс (1.0 мс) | Обычный (Normal) | Вторичное ядро (защита от опустошения буфера) |
| **Стандартная периферия (флешки, вебкамера)** | Любой доступный порт | `0xC8` (По умолчанию) | 50 мкс | Обычный (Normal) | Общий пул ядер планировщика |
...
<p align="center">
  <img src="./DEVICE%20TWEAKER/assets/screenshots/showcase_nic_itr_ru.png" alt="Управление регистрами NIC ITR и распределением очередей RSS" width="100%">
</p>

#### Практические примеры конфигурации сетевых адаптеров

| Адаптер | Режим ITR | Значение регистра | Число очередей RSS | Распределение ядер (Affinity) | Результат |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Intel Ethernet I225-V / I226-V (2.5G)** | Отключено (Zero Delay) | `0x0, 0x0, 0x0, 0x0` | 4 очереди (`*NumRssQueues=4`) | Выделенный блок ядер (CPU 12-15) | Мгновенный отклик сетевых пакетов без джиттера, ноль задержки в буфере |
| **Realtek RTL8125 / RTL8126 (2.5G/5G)** | Отключено (Zero Delay) | `0x0` (`0x00000000`) | 2-4 очереди | Вторичные ядра CCD0 / P-ядра | Полное исключение модерации прерываний на чипах Dragon/Gaming |
| **Intel I219-V / I211-AT (1G)** | Фиксированная задержка | `0x3E8` (1000 мкс) | 2 очереди | Вторичный чиплет CCD1 / E-ядра | Баланс между минимальным сетевым оверхедом CPU и плавной передачей потока |
